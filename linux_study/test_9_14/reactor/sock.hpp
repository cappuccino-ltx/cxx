...
public:
    ...
    static void Bind(int listenSock, std::string ip, int16_t port)
    {
    ...
    static int Accept(int listenSock, std::string &ip, uint16_t &port,int &accept_errno)
    {
        ...
        if (serviseSock > 0)
        {
            ...
            logmessage(NORMAL, "%s:%d 已成功连接访问", ip.c_str(), port);
        }
        else {
            accept_errno = errno;
        }
    ...
    static void Connect(int serverSock, std::string serverip, int16_t serverport)
    {
        // 获取链接
    ...
    static void Send(int sock, const std::string str)
    {
        ...
        {
            logmessage(ERROR, "向socket：%d发送失败，发送内容：%s", sock, str.c_str());
        }
    }
    static bool Recv(int sock, std::string &out)
    {
        char buffer[1024];
        ...
        {
            logmessage(NORMAL, "连接%d已断开连接！", sock);
            close(sock);
            return false;
        }
        else
